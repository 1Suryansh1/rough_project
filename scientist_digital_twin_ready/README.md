...
> **Target scientist (default): Richard Feynman.** Swap `wikidata_qid`, `semantic_scholar_id`, and `wikipedia_title` in `config/scientist.yaml` for any other scientist.
...
│  Anthropic Claude API (one call per turn)               │
...
│  VibeVoice 1.5B LoRA fine-tune (community fork)         │
│  Fallback: XTTS v2 few-shot enrollment                  │
└─────────────────────────────────────────────────────────┘
```
...
| `generate.py` | **Only LLM call** — Anthropic Claude API |
...
| `setup_vibevoice_finetune.py` | Clones community fork, prepares LJSpeech dataset |
| `finetune_vibevoice.sh` | Launches LoRA fine-tune (≥16GB VRAM required) |
| `synthesize.py` | Synthesis via VibeVoice (primary) or XTTS v2 (fallback) |
...
| VibeVoice LoRA fine-tune | 16GB VRAM | 24GB VRAM |
| XTTS v2 synthesis (fallback) | 4GB VRAM | 8GB VRAM |
...
# Requires ANTHROPIC_API_KEY + bert-score installed
...
- **Voice audio quality**: Cornell 1964 Messenger Lectures may have low SNR. Check `data/processed/voice_manifest.json` for `total_audio_minutes` — if < 30 min of clean speech, XTTS v2 fallback is recommended.
- **VibeVoice community fork**: Check the fork's README for the exact `finetune.py` CLI flags — they may differ slightly from what's in `finetune_vibevoice.sh`.
...
Research use. All third-party data sources (Wikidata, Semantic Scholar, Wikipedia, Nobel Prize) are used under their respective open licenses. VibeVoice community fork is MIT-licensed.
